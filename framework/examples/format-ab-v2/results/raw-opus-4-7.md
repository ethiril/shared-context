...
[
  ...
  },
  {
    ...
    "done": [
      "POST /signup enqueues against welcome-email-job v1.0.0",
      "ADR for fire-and-forget published",
      "unit + integration tests green"
    ],
    "next": [
      "joint staging signup test once worker confirms consuming",
      "nothing else for v1"
    ],
    "blocked_on": [],
    "contracts_in_play": [
    ...
    "open_questions": [
      {"to": "worker", "ask": "confirm consumer wired & non-`en` locales fall through cleanly"}
    ]
  },
  {
    "type": "status",
    ...
    "current_goal": "Consume welcome-emails and send via SES",
    "done": [
      "consumer for welcome-emails queue",
      "payload validation against v1.0.0 with bad-shape→DLQ",
      "non-`en` locales fall through to English with debug log",
      "end-to-end staging green"
    ],
    "next": [
      "v2 retry policy when scoped",
      "nothing pending for v1"
    ],
    "blocked_on": [],
    "contracts_in_play": [
      {"name": "welcome-email-job", "version": "1.0.0", "role": "consumer"}
...
[
  ...
  },
  {
    ...
    "alternatives_rejected": [
      {"option": "synchronous send from API", "reason": "SES p99 well above signup-latency budget"},
      {"option": "job-status callback to API", "reason": "no consumer for v1"},
      {"option": "outbox pattern", "reason": "overkill until we have a transactional concern"}
    ],
    "consequences": [
      "worker is source of truth for \"did the email send\"",
...
None. All batches produced the expected element count:
- Batch 1: 6 elements (2 short-key + 2 positional + 2 DSL) ✓
- Batch 2: 1 object ✓
- Batch 3: 6 elements (2 short-key + 2 positional + 2 DSL) ✓
- Batch 4: 3 elements (1 each) ✓
- Batch 5: 3 elements (1 each) ✓
- Batch 6: 1 object ✓

Notes on DSL parsing (the least-precedented format):
- For DSL log entries (batch 1, items 5–6), the `summary` was taken as the text between `:` and the first `|`; the `body` is the trailing free-form section after the final `|`. The summaries are shorter than the short-key/positional versions because the DSL encoded a more condensed form — this is faithful to the source, not hallucinated.
- For the DSL repos artefacts (batch 3, items 5–6), `contracts` shorthand `name@version(role[; consumer=...])` was expanded into the canonical object form. The worker entry has no `consumer=` sub-field, so it's omitted (not invented).
- For the DSL contract (batch 4, item 3), `queue.backend` is `Redis-bull` per the source — the short-key/positional variants spell it out as `Redis-backed bull queue (existing)`. Extracted faithfully.
- For the DSL ADR (batch 5, item 3), the `affects` is parsed as the single token `welcome-email-job` from `affects:<scope>`; `why` was extracted as the three semicolon-separated clauses (rephrased more tersely in the DSL source than in short-key/positional). Reproduced verbatim from the DSL, not normalized to the other formats' wording.
