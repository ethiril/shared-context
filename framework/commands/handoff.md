...
| ≥10 | write without asking |
| no digest exists yet **and** > 0 substantive events (`[q]`/`[a]`/`[ch]`/`[cc]`/`[bl]`/`[pv]`/`[tk]`) | write without asking |
| no digest exists yet **and** only handshake events (`[fy]`/`[ak]`) | skip — tell user "first digest deferred; nothing substantive landed yet." |
...
### 3. Idle positional (when your part is done but the feature isn't)

If you've finished your work on this feature but the whole feature isn't closing (other repos still owe work, deploy pending, etc.), write a fresh `repos/<self>/<UTC-ISO-timestamp>.positional` with `current_goal: idle — waiting on <repo|event>`, empty `next`, and `blocked_on: <one-phrase blocker>`. The dashboard surfaces `blocked_on` automatically. See `framework/CONVENTIONS.md` → "Signalling I'm done with my part."

Skip this step if you're mid-work and just clearing context — your existing positional already reflects in-progress state.
